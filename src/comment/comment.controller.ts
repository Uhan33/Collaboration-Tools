...
export class CommentController {
  ...
  @Post()
  createComment(@Body() createCommentDto: CreateCommentDto, @UserInfo() user: User) {
    return this.commentService.createComment(createCommentDto, user.id);
  }

  @Get()
  findAllCommentByCardId(@Query('cardId') cardId: number, @UserInfo() user: User) {
    return this.commentService.findAllCommentByCardId(cardId, user.id);
  }

  @Patch(':id')
  updateComment(@Param('id') id: number, @Body() updateCommentDto: UpdateCommentDto, @UserInfo() user: User) {
